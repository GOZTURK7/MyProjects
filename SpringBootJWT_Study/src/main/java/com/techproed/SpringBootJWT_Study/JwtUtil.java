...
@Service
public class JwtUtil { // bir depo gibi sık kullanılan methodların saklandığı classtır. interview de mutlaka project anlatılırken sorulur.
    ...
    public Boolean validateToken(String token, UserDetails userDetails){//App create ettiği token ile client'den gelen token  oney controlu
        final String username=extractUserName(token); // token extractUserName() method call ederek return olan username'i string type'da variable'a ata
