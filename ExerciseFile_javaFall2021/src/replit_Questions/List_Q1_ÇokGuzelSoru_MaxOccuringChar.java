...
public class List_Q1_ÇokGuzelSoru_MaxOccuringChar {
...
//		System.out.println(input1);
		...
		for (int i = 0; i < uzunluk; i++) {
			ctr[input.charAt(i)]++; // ctr arrayinde "input1.get(i)" stringinden aldığımız 
		...
		int max = -1; // -1 yaptık çünkü ctr array'indeki default değer 0 veya aynı karakter vardıysa > 0 bir rakam
		
		char sonuc=' '; // 
		
		for (int i = 0; i < uzunluk; i++) {
			if (ctr[input.charAt(i)]>max) { // ctr içinde "input1.get(0).charAt(0)" a denk gelen char'ın sayısal
